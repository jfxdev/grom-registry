...
<template>
  <main class="page-shell">
    ...
    <section v-else-if="status.data.value" class="settings-grid">
      ...
      <Card class="status-card"><Trash2 :size="22" class="text-accent" /><div><p class="status-label">Garbage collection</p><p class="status-value">Reclaim deleted blobs</p><p class="mb-3 text-sm text-muted-foreground">Writes pause briefly while Distribution collects unreferenced data.</p><Button size="sm" :loading="garbageCollection.isPending.value" :disabled="status.data.value.distribution !== 'available'" @click="garbageCollection.mutate()">Run garbage collection</Button><p v-if="garbageCollection.data.value" class="mt-2 text-sm text-muted-foreground">Reclaimed {{ formatBytes(garbageCollection.data.value.reclaimedBytes) }}.</p><p v-if="garbageCollection.isError.value" class="mt-2 text-sm text-destructive">Garbage collection could not complete.</p></div></Card>
    </section>
  </main>
</template>
