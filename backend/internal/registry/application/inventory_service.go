...
func (s *InventoryService) ListPage(ctx context.Context, projectID foundation.ID, repository, query string, request foundation.PageRequest) (foundation.PageResult[registrydomain.ManifestInventory], error) {
	target, err := s.store.FindRepository(ctx, projectID, repository)
	if err != nil {
		return foundation.PageResult[registrydomain.ManifestInventory]{}, err
	}
	return s.store.ListManifestInventoryPage(ctx, target.ID, query, request)
}
